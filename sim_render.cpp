...
std::array<std::array<float, 6>, 10000> inputdata;//i rows, j cols
...
std::ifstream datfile("savedData.txt");
...
{
    int switches[2] = {1};
	forceAdj = inputdata[counter][5];//force data from savedData file of previous sim run 
	ctrl[0] = forceAdj; // not exactly the same as forceIn from sim because its been adjusted
	ctrl[1] = 0*inputdata_inforces[counter][1];//knee extensor force
	ctrl[2] = 0*inputdata_inforces[counter][2];//hip extensor force
	//printf("knee force %f, hip force %f\n", inputdata_inforces[counter][1], inputdata_inforces[counter][2]);
    // for( int i=0; i<nu; i++ )
        //qfrc_applied[i] = mju_sin(time);
        //qfrc_applied[i] = -1* switches[i];
        // ctrl[i] = forceIn; // not exactly the same as forceIn from sim because its been adjusted
	// if (counter == 0) {
		// printf("forceAdj, %f\n", ctrl[0]);
	// }
	// printf("%f\n", ctrl[0]);
...
{
	//for reading file!!
	// std::array<std::array<float, 6>, 3000> inputdata;//i rows, j cols
	// std::ifstream datfile("savedData.txt");

	for (int i = 0; i <= max_steps; i++) {
		for (int j = 0; j <= 6; j++) {
			datfile >> inputdata[i][j];
		}
	}
	...
	loadmodel(window, "3SegRMdevel01.xml", 0);
	printf("hello \n");
    // main loop
    ...
    while( !glfwWindowShouldClose(window) && counter < max_steps)
    {
        // simulate and render
        render(window);
		if (counter == 0) {

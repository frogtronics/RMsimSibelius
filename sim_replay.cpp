...
std::array<std::array<float, 10>, 1000> inputdata_kinematics;//3 input forces, 1000 rows
std::ifstream datfile_kinematics("savedData_kinematics.txt");
...
void replay()
{
	for (int i = 0; i < m->nq; i++) {
		m->qpos0[i] = inputdata_kinematics[counter][i];
	}
}
...
int main(int argc, const char** argv)
{
	...
	for (int i = 0; i < 1000; i++) {
		for (int j = 0; j < 10; j++) {
			datfile_kinematics >> inputdata_kinematics[i][j];
		}
	}
	...
	for (int i = 0; i<=m->nq; i++) {
		m->qpos0[i] = inputdata_kinematics[0][i];
	}	
	//now account for free joint
	for (int i = 0; i < 7; i++) {
		d->qpos[i] = inputdata_kinematics[0][i];
	}
